...
def data_splitter(path_to_data, destination_dir, number_of_splits = 5, seed = 42): 
    ...
    dataset = dc.data.NumpyDataset(X = data.id,
                                y = data.rt,
                                ids = data.smiles)

    splitter = dc.splits.ScaffoldSplitter()

    train_dataset, test_dataset = splitter.train_test_split(dataset=dataset, frac_train = 0.9, seed = seed)
    ...
    for i,k_split in enumerate(cv_splits):
        ...
        train_k_split = train_k_split.rename(columns = {'X':'id','y':'rt','ids':'smiles'})
        train_k_split.to_csv(os.path.join(target_dir,f'train_{i}_split.csv'),index = False)
        
        valid_k_split = k_split[1]
        valid_k_split = valid_k_split.to_dataframe()
        valid_k_split = valid_k_split.rename(columns = {'X':'id','y':'rt','ids':'smiles'})
        valid_k_split.to_csv(os.path.join(target_dir,f'valid_{i}_split.csv'),index = False)
...
def feature_splitter_csv(path_to_features,path_to_splits, save_folder_name = 'data_splits'):
    ...
    for i, path in enumerate(csv_feature_paths):
        ...
        for j, split in enumerate(split_paths):
            ...
            merged_split = pd.concat([split, features], axis=1, join='inner')#, on='id')
